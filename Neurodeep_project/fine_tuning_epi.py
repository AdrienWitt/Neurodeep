# -*- coding: utf-8 -*-
"""
Created on Wed Jun 29 14:12:29 2022
...
for path, folders, files in os.walk(path1):
    for file in files:
        if fnmatch.fnmatch(file, '*train.npy*'):
            control_list.append(os.path.join(path, file))

for path, folders, files in os.walk(path2):
    for file in files:
        if fnmatch.fnmatch(file, '*train_epi.npy*'):
            epi_list.append(os.path.join(path, file))
...
for file in epi_list:
    if fnmatch.fnmatch(file, '*train_epi.npy*' ):
            label = 1
            labelepi_list.append(label)



train_l = control_list[0:280] + epi_list[0:290]
train_lab = label_list[0:280] + labelepi_list[0:290]

val_l = control_list[280:320] + epi_list[290:330]
val_lab = label_list[280:320] + labelepi_list[290:330]

test_l = control_list[320:400] + epi_list[330:420]
test_lab = label_list[320:400] + labelepi_list[330:420]
